...
(defmacro with-dynamic-extent-charseq ((charseq string &key (start 0) (end '(common-lisp:length string)))
				       &body body)
  (multiple-value-bind (str-var beg-var end-var) (#1=(gensym)#1##1#)
    `(let ((str-var ,string)
	   (beg-var ,start)
	   (end-var ,end))
	   ...
	   (declare (dynamic-entent ,charseq))
	   ,@body)))))
